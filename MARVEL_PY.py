...
cursor = conn.cursor()
cursor.execute(f"""
CREATE TABLE IF NOT EXISTS fabianteseyra_coderhouse.marvel (
    name VARCHAR(1000),
    comics VARCHAR(1000),
    series VARCHAR(1000),
...
# %%
cursor.executemany("""
    INSERT INTO fabianteseyra_coderhouse.marvel (name, comics, series, description)
    VALUES (%s, %s, %s, %s)
""", data)
...
try:
    ...
    cursor.execute(f"""
    ALTER TABLE fabianteseyra_coderhouse.marvel
...
for index, row in df.iterrows():
    ...
    cursor.execute("""
        UPDATE fabianteseyra_coderhouse.marvel
